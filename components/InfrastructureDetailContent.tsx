...
export default function InfrastructureDetailContent({ slug }: { slug: InfrastructureSlug }) {
  ...
  return (
    <div className="bg-white selection:bg-[#10b981] selection:text-white">
      {/* Hero Section */}
      <section className="relative py-12 md:py-20 lg:py-24 pt-16 md:pt-20 overflow-hidden">
      ...
      <section className="relative z-20 px-3 md:px-6 lg:px-12 -mt-8 md:-mt-10 pb-12 md:pb-24 lg:pb-32">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 lg:gap-8">
          {featureCards.map((feature, i) => (
            <div
              key={i}
              className="bg-white p-4 md:p-8 lg:p-10 xl:p-12 rounded-xl md:rounded-[2.5rem] shadow-2xl border border-gray-50 group hover:-translate-y-4 transition-all duration-500 flex flex-col items-center text-center"
            >
              <div className="w-12 h-12 md:w-20 md:h-20 rounded-lg md:rounded-2xl bg-[#f0fdf9] flex items-center justify-center text-[#10b981] mb-4 md:mb-8 group-hover:bg-[#10b981] group-hover:text-white transition-all">
                <div className="scale-75 md:scale-100">{feature.icon}</div>
              </div>
              <h3 className="text-sm md:text-2xl font-black tracking-tight mb-2 md:mb-4 text-[#111]">{feature.title}</h3>
              <p className="text-gray-500 text-[10px] md:text-base font-medium leading-relaxed line-clamp-3 md:line-clamp-none">{feature.desc}</p>
            ...
            <div className="rounded-2xl md:rounded-[4rem] overflow-hidden shadow-2xl relative aspect-[4/5] lg:aspect-square">
              ...
              <div className="absolute bottom-[-10px] left-4 md:left-10 bg-white p-4 md:p-8 rounded-xl md:rounded-[2.5rem] shadow-2xl border border-gray-50 animate-float-slow">
                <div className="flex items-center space-x-2 md:space-x-4">
                  ...
                  <div>
                    <div className="text-lg md:text-3xl font-black text-[#111]">10+</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 lg:gap-8 max-w-7xl mx-auto">
          {detailedServices.map((service, i) => (
            <div key={i} className="group relative rounded-xl md:rounded-[3rem] overflow-hidden aspect-[5/3] md:aspect-[4/5] shadow-2xl">
              ...
              <div className="absolute bottom-2 left-2 right-2 md:bottom-10 md:left-10 md:right-10 space-y-1 md:space-y-4">
                <span className="text-[6px] md:text-[10px] font-black uppercase text-[#10b981] tracking-[0.2em] md:tracking-[0.4em]">
                  Service_ID: 0{i + 1}
                </span>
                <h4 className="text-xs md:text-3xl font-black text-white tracking-tight leading-none line-clamp-1">{service.title}</h4>
                <p className="text-white/60 text-[8px] md:text-sm font-medium line-clamp-2 md:line-clamp-2">{service.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-12 md:py-24 lg:py-32 px-3 md:px-6 lg:px-12 bg-[#fafafa] border-y border-gray-100">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-center gap-6 md:gap-8 lg:gap-12 xl:gap-24">
          {stats.map((stat, i) => (
            <div key={i} className="flex flex-col items-center space-y-3 md:space-y-6">
              <div className="w-24 h-24 md:w-40 md:h-40 lg:w-48 lg:h-48 rounded-full border-2 border-gray-100 bg-white shadow-xl flex items-center justify-center relative group">
                <div className="absolute inset-2 md:inset-4 rounded-full border-2 md:border-4 border-[#10b981]/10 group-hover:border-[#10b981]/40 transition-colors" />
                <div className="text-lg md:text-3xl lg:text-4xl xl:text-5xl font-black tracking-tighter text-[#111]">{stat.value}</div>
              </div>
              <div className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-gray-400 text-center max-w-[100px] md:max-w-[120px]">
                {stat.label}
              </div>
            </div>
          ))}
